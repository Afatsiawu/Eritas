...
export function MusicProvider({ children }: { children: ReactNode }) {
    ...
    const addSong = useCallback(async (track: Track, userId: string) => {
        if (!activeTrip) return;

        try {
            const apiUrl = process.env.NEXT_PUBLIC_API_URL || "http://localhost:3000";
            const response = await fetch(`${apiUrl}/spotify/queue`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
                    busId: "bus-1", // Hardcoded for demo
                    trackUri: `spotify:track:${track.id}`,
                    userId: 1 // Hardcoded ID matching backend seed or assumption
                })
            });
